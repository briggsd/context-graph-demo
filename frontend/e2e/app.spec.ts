...
test.describe("Software Engineering Context Graph", () => {
  ...
  test("backend health check returns ok or degraded", async ({ request }) => {
    ...
    expect(body.domain).toBe("software-engineering");
  });
  ...
  test("demo prompt: Code Review & PRs — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("Show me all open pull requests across our repositories");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("Show me all open pull requests across our repositories").first()).toBeVisible();
  ...
  test("demo prompt: Incident Response — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("Are there any active incidents? What services are affected?");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("Are there any active incidents? What services are affected?").first()).toBeVisible();
  ...
  test("demo prompt: Engineering Intelligence — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("Which services have the most dependencies?");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("Which services have the most dependencies?").first()).toBeVisible();
  ...
  test("tool calls show timeline with status indicators", async ({ page }) => {
    ...
    const toolBadge = page.locator("[data-scope='badge']").filter({ hasText: /execute_cypher|get_schema|search_developer/ });
  ...
  test("API: Code Review & PRs prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me all open pull requests across our repositories" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Code Review & PRs prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Who are the most active code reviewers this sprint?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Code Review & PRs prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What issues are blocked or in review right now?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Incident Response prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Are there any active incidents? What services are affected?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Incident Response prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me the deployment history for the auth service this week" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Incident Response prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What was the root cause of the last sev1 incident?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Engineering Intelligence prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Which services have the most dependencies?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Engineering Intelligence prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What\u0027s the relationship between recent deployments and incidents?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Engineering Intelligence prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Find similar past incidents to a current database performance issue" },
    });
    expect(res.ok()).toBeTruthy();
